...
class Music(commands.Cog):
    ...
    def get_ydl_opts(self):
        ...
        # Добавляем прокси, если указан в переменной окружения
    ...
    async def play_track(self, interaction, url):
        ...
        if cached:
            ...
            try:
                ...
                if "Sign in to confirm you’re not a bot" in str(e):
                    error_msg = "Это видео требует входа в аккаунт YouTube. Попробуйте общедоступное видео или используйте другой источник (например, SoundCloud)."
                elif "Requested format is not available" in str(e):
                    error_msg += " Запрошенный формат недоступен. Попробуйте другое видео."
                try:
        ...
        if "entries" in info:
            ...
            try:
                ...
                if "Sign in to confirm you’re not a bot" in str(e):
                    error_msg = "Это видео требует входа в аккаунт YouTube. Попробуйте общедоступное видео или используйте другой источник."
                elif "Requested format is not available" in str(e):
                    error_msg += " Запрошенный формат недоступен. Попробуйте другое видео."
                try:
                    await interaction.followup.send(error_msg)
            ...
            try:
                ...
                if "Sign in to confirm you’re not a bot" in str(e):
                    error_msg = "Это видео требует входа в аккаунт YouTube. Попробуйте другой источник или общедоступное видео."
                elif "Requested format is not available" in str(e):
                    error_msg += " Запрошенный формат недоступен. Попробуйте другое видео."
                try:
                    await interaction.followup(error_msg)
                except Exception as followup_e:
                    logger.error(f"Ошибка при отправке followup: {e}")
                return
        ...
        ffmpeg_options = {
            ...
            'options': '-vn -bufsize 1M'  # Увеличен буфер для стабильности
        }
    ...
    async def nowplaying(self, interaction: discord.Interaction):
        ...
        try:
            if title:
                await interaction.response.send_message(f"Сейчас сейчас: **{title}**")
            else:
